...
describe("Tree API test", () => {
  beforeAll(async () => {
    ...
    execSync(
      `DATABASE_URL=${process.env.TEST_DATABASE_URL} prisma migrate deploy`
    );
  ...
  describe("POST /api/tree", () => {
    it("It should create a root node", async () => {
      ...
      const nodeWithLabel = createdNodes.find(
        (obj) => obj.label === treeNode.label
      );
    ...
    it("It should create a node with given parentId", async () => {
      ...
      // checking if the expected node exist or not
      const nodeWithLabelAndParent = createdNodes.find(
        (obj) =>
          obj.label === nodeWithParent.label &&
